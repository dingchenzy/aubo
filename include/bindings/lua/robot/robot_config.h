#ifndef AUBO_SCRIPT_BINDING_LUA_ROBOT_ROBOT_CONFIG_H
#define AUBO_SCRIPT_BINDING_LUA_ROBOT_ROBOT_CONFIG_H
...
class LuaRobotConfig
{
public:
    LuaRobotConfig(const RobotConfigPtr &_impl) : self(_impl) {}
...
#undef _FUNC
    ...
    static void registerLuaUserType(sol::state_view &L)
    {
        ...
        auto u = L.new_usertype<LuaRobotConfig>("RobotConfig", enrollments);

#define _INST(m, n, f, ...)                 \
    u.set_function(#f, &LuaRobotConfig::f); \
    wrapper_block1(L, "RobotConfig", #f);
#define _FUNC(m, n, f, ...)                 \
    u.set_function(#f, &LuaRobotConfig::f); \
    wrapper_block2(L, "RobotConfig", #f);
        RobotConfig_DECLARES;
#undef _INST
#undef _FUNC
    }
...
using LuaRobotConfigPtr = std::shared_ptr<LuaRobotConfig>;
} // namespace aubo_script
} // namespace arcs
#endif
