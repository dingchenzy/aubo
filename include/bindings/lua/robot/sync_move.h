#ifndef AUBO_SCRIPT_BINDING_LUA_ROBOT_SYNC_MOVE_H
#define AUBO_SCRIPT_BINDING_LUA_ROBOT_SYNC_MOVE_H
...
{
public:
    LuaSyncMove(const SyncMovePtr &_impl) : self(_impl) {}
...
#undef _FUNC
    ...
    static void registerLuaUserType(sol::state_view &L)
    {
        ...
        auto u = L.new_usertype<LuaSyncMove>("SyncMove", enrollments);

#define _INST(m, n, f, ...)              \
    u.set_function(#f, &LuaSyncMove::f); \
    wrapper_block1(L, "SyncMove", #f);
#define _FUNC(m, n, f, ...)              \
    u.set_function(#f, &LuaSyncMove::f); \
    wrapper_block2(L, "SyncMove", #f);
        SyncMove_DECLARES;
#undef _INST
#undef _FUNC
    }
...
using LuaSyncMovePtr = std::shared_ptr<LuaSyncMove>;
} // namespace aubo_script
} // namespace arcs
#endif
